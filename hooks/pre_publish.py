...
class PrePublishHook(Hook):
    ...
    """
    def execute(self, engine_name, tasks, work_template, progress_cb, **kwargs):
        """
        Main hook entry point
        """
        pre_publisher = PrePublisher.create(engine_name, self.parent)
        if not pre_publisher:
            raise Exception("Don't know how to pre-publish tasks for engine: %s" % engine_name)
        return pre_publisher.execute(tasks, work_template, progress_cb, **kwargs)

        
class PrePublisher(object):
    """
    Base class for engine specific pre-publishers
    """
    
    @staticmethod
    def create(engine_name, parent):
        """
        Helper factory method to create a publisher instance for
        the given engine
        """
        pre_publisher_classes = {"tk-maya":MayaPrePublisher, 
                                 "tk-nuke":NukePrePublisher}
        cls = pre_publisher_classes.get(engine_name)
        if cls:
            return cls(parent)
    ...
    def execute(self, tasks, work_template, progress_cb, **kwargs):
        ...
        Does nothing
        """
        raise NotImplementedError("Pre-publish not implemented")
...
class NukePrePublisher(PrePublisher):
    """
    Engine specific pre-publisher for Nuke
    """
    def execute(self, tasks, work_template, progress_cb, **kwargs):
        """
...
class MayaPrePublisher(PrePublisher):
    """
    Engine specific pre-publisher for Maya
    """
    def execute(self, tasks, work_template, progress_cb, **kwargs):
        """
